...
if(isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if($stmt->rowCount() > 0) {
        ...
        $titre = $services['titre'];
        $description = $services['descriptions'];
        $prix= $services['prix'];
        $photo_name = $services['photo'];
    } 
}
...
if (!empty($_FILES['photo']['name'])) {
    ...
    $stmt = $pdo->prepare("UPDATE services SET titre = ?, photo = ?, descriptions = ?, prix = ? WHERE id = ?");
    $stmt->execute([$titre, $photo_name, $description, $prix, $id]);
...
<div class="row">
<div class="col-12">
<h5 class="form-title student-info">Information de l'services <span><a href="javascript:;"><i class="feather-more-vertical"></i></a></span></h5>
</div>
...
<div class="form-group local-forms">
<label>prix </label>
<input class="form-control" type="text" placeholder="Entrer le titre" name="prix"  value="<?php echo $prix; ?>">
</div>
</div>
